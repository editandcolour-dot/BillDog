/**
 * Tier 3 Blind Runner — processes all T3 edge-case bills through Billdog's full pipeline
...
const TIER3_DIR = path.join(__dirname, '../tests/corpus/v5/billdog-tier3-bills/tier3_edge_cases');
const OUTPUT_PATH = path.join(__dirname, 'tier3-raw-output.json');

async function main() {
  console.log('--- Tier 3 Blind Runner ---');

  const files = fs.readdirSync(TIER3_DIR)
    .filter(f => f.endsWith('.pdf'))
    .sort();

  console.log(`Found ${files.length} Tier 3 bills.`);
  ...
  for (const filename of files) {
    ...
    const pdfPath = path.join(TIER3_DIR, filename);
    ...
    try {
      ...
      result.parsed_bill_summary = {
        ...
        valuation: parsedBill.valuation,
        vatAmount: parsedBill.vatAmount,
      };
    } catch (err: any) {
    ...
    try {
      ...
      result.validator_findings = findings.map((f: any) => ({
        type: f.type,
    ...
    try {
      ...
      result.analysis_errors = analysis.errors.map((e: any) => ({
  ...
  // Output
  const output = {
    runner: 'tier3-blind-runner',
    timestamp: new Date().toISOString(),
    total_bills: results.length,
    bills_with_findings: results.filter((r: any) => r.analysis_errors.length > 0).length,
    bills_clean: results.filter((r: any) => r.analysis_errors.length === 0).length,
    results
  };

  fs.writeFileSync(OUTPUT_PATH, JSON.stringify(output, null, 2));
  console.log(`\n=======================================`);
  console.log(`TIER 3 BLIND RUN COMPLETE`);
  console.log(`=======================================`);
